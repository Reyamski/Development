...
import { fetchTopStatements, fetchTopConsumers, fetchCloudWatchIops, fetchRdsConfig, fetchInnodbMetrics, fetchParameterGroup, awsSsoStatus } from '../api/client';
...
export function useIops() {
  ...
  const refresh = useCallback(async () => {
    ...
    // Proactive AWS SSO check — server returns empty arrays on SSO miss instead of throwing,
    // so the amber re-login prompt never fires from the catch block. Drive it from sso-status.
    if (hasAws) {
      try {
        const { loggedIn } = await awsSsoStatus();
        useAppStore.getState().setAwsAuthRequired(
          !loggedIn,
          loggedIn ? '' : 'AWS SSO session is not active. Re-login to fetch CloudWatch and RDS data.'
        );
      } catch {
        // Non-fatal — let downstream fetches surface their own errors
      }
    }
