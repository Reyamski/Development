...
async function readApiJson<T>(response: Response): Promise<T> {
  ...
  if (trimmed.startsWith('<') || trimmed.startsWith('<!')) {
    throw new Error(
      'API returned a web page instead of JSON - the backend on port 3005 is probably not running.'
    );
  }
...
export const runAnalysis = (database: string) =>
  postJson<AnalysisResults>('/analysis/run', { database });
