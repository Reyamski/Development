...
const WINDOWS_TSH_CANDIDATES = [
  path.join(
    ...
    'WinGet',
    'Packages',
    'Gravitational.Teleport_Microsoft.Winget.Source_8wekyb3d8bbwe',
    'tsh.exe',
  ),
...
export async function findTsh(override?: string): Promise<string> {
  ...
  // Use the platform-native command so tsh can be discovered on both Windows and Unix-like systems.
  try {
    const resolver = process.platform === 'win32' ? 'where' : 'which';
    const { stdout } = await execFileAsync(resolver, ['tsh']);
    const tshPaths = stdout
      .split(/\r?\n/)
      .map(line => line.trim())
      .filter(Boolean);
    ...
    for (const tshPath of tshPaths) {
      try {
        await fs.access(tshPath, fs.constants.F_OK);
        return tshPath;
      } catch {
        // Keep checking the remaining PATH results.
      }
    }
  } catch {
    // not in PATH
  ...
  if (process.platform === 'darwin') {
    try {
      await fs.access(TELEPORT_CONNECT_TSH, fs.constants.F_OK);
  ...
  if (process.platform === 'win32') {
    for (const tshPath of WINDOWS_TSH_CANDIDATES) {
      try {
        ...
        // Keep checking the remaining Windows install locations.
      }
    }
  }
