...
const selectClasses = 'w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-sm text-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none';
const labelClasses = 'block text-xs font-medium text-slate-500 uppercase tracking-wider mb-1';
    ...
    <div className="space-y-3">
      ...
      {store.selectedCluster && !isLoggedIn && (
        <div className="flex items-center gap-3">
          <button
            onClick={login}
            className="w-full py-2.5 text-sm font-medium rounded bg-indigo-600 hover:bg-indigo-500 text-white transition-colors"
          >
            Login via SSO
          </button>
        </div>
      ...
      {isLoggedIn && (
        <div className="flex items-center gap-2 px-3 py-2 rounded bg-slate-800 border border-slate-700">
          ...
          <span className="text-xs text-slate-300">
            Logged in as <span className="text-slate-100">{store.loginStatus!.username}</span>
          </span>
        </div>
      )}
      ...
      {isLoggedIn && store.instances.length > 0 && (
        <div>
          <label className={labelClasses}>RDS Instance (Replica)</label>
          <select
            className={selectClasses}
      ...
      {isConnected && (
        <div className="rounded bg-slate-800 border border-slate-700 px-3 py-3 space-y-2">
          ...
          <div className="text-xs text-slate-400 space-y-1">
            <div>Cluster: <span className="text-slate-200">{store.selectedCluster}</span></div>
            <div>Instance: <span className="text-slate-200">{store.selectedInstance}</span></div>
            <div>Version: <span className="text-slate-200">{store.connectionResult!.version}</span></div>
            {store.rdsConfig && (
              <div>Class: <span className="text-slate-200">{store.rdsConfig.instanceClass}</span></div>
            )}
          </div>
        </div>
      )}
