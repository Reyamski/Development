...
const WINGET_PACKAGES_DIR = path.join(os.homedir(), 'AppData', 'Local', 'Microsoft', 'WinGet', 'Packages');
...
/**
 * Locate the tsh binary. Checks PATH first, then platform-specific fallback locations.
...
export async function findTsh(override?: string): Promise<string> {
  ...
  // Check PATH via `where` (Windows) or `which` (Mac/Linux)
  try {
    const { stdout } = await execFileAsync(isWindows ? 'where' : 'which', ['tsh']);
    const tshPath = stdout.split('\n')[0].trim();
  ...
  if (isWindows) {
    ...
    try {
      const entries = await fs.readdir(WINGET_PACKAGES_DIR);
      const teleportDir = entries.find(e => e.startsWith('Gravitational.Teleport_'));
      if (teleportDir) {
        const tshPath = path.join(WINGET_PACKAGES_DIR, teleportDir, 'tsh.exe');
        await fs.access(tshPath);
        return tshPath;
      }
    } catch {
      // not found
  ...
  throw new Error('Could not find tsh binary. Install Teleport or Teleport Connect.');
}
