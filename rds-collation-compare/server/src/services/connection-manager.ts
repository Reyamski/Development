...
/**
 * Manages a persistent tunnel + MySQL connection for index analysis queries.
...
/**
 * Get the active MySQL connection. Returns null if not connected.
 */
...
/**
 * Execute a read-only query. Throws if sql contains mutating keywords.
 */
export async function executeReadOnly(sql: string, params: any[] = []): Promise<any> {
